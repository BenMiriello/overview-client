...
export class CloudLayer extends BaseLayer<void> {
  ...
  update(): void {
    ...
    for (const { mesh, altMultiplier } of this.shells) {
      ...
      const densityLo = getConfig<number>('layers.clouds.densityLo') ?? 0.05;
      mat.uniforms.uDensityLo.value = densityLo;
      sharedNightUniforms.cloudDensityLo.value = densityLo;
